...
namespace hr {
    ...
    public:
        ...
        static Result<Intergrowth> from_step_function(step_function<T> const &step_function, T starting_grade,
                                                      T final_grade) {
            const Result<point_bound_list<T>> p_and_b = points_and_bounds<T, Guarantees...>(step_function, starting_grade, final_grade);
    ...
    private:
        ...
        explicit Intergrowth(cubic_spline<T> && initial, point_bound_list<T> && points_and_bounds) : initial(initial), _points_and_bounds(points_and_bounds) {}
